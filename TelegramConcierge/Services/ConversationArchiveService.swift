...
actor ConversationArchiveService {
    ...
    Never preserve assistant reasoning, chain-of-thought, draft text, hidden scratch work, or text inside <think> tags. If such text appears in source material or model output, omit it from summaries, user-context updates, excerpt extraction, and meta-summaries.
    ...
    private func generateSummary(for messages: [Message], startDate: Date, endDate: Date, context: SummarizationContext) async throws -> String {
        ...
        Do not preserve assistant reasoning, chain-of-thought, draft text, hidden scratch work, or text inside <think> tags.
    ...
    private func extractAndAppendUserContext(messages: [Message], startDate: Date, endDate: Date, context: SummarizationContext) async {
        ...
        - Assistant reasoning, chain-of-thought, draft text, hidden scratch work, or text inside <think> tags
    ...
    private func restructureUserContext() async {
        ...
        - Remove assistant reasoning, chain-of-thought, draft text, hidden scratch work, or text inside <think> tags
    ...
    private func callLLM(systemPrompt: String, userPrompt: String, maxTokens: Int? = nil, sharedContextPrompt: String? = nil) async throws -> String {
        ...
        return stripInlineReasoningBlocks(from: decoded.choices.first?.message.content ?? "")
    }
    ...
    private func formatMessagesForSummary(_ messages: [Message]) async -> String {
        ...
        for msg in messages {
            ...
            let content = stripInlineReasoningBlocks(from: await decorateMessageContentForArchive(msg.content, message: msg))
    ...
    private func loadIndex() {
        ...
        do {
            ...
            if sanitizeStoredArchiveSummaries() > 0 {
                saveIndex()
            }
    ...
    @discardableResult
    private func sanitizeStoredArchiveSummaries() -> Int {
        var changed = 0

        for index in chunkIndex.chunks.indices {
            let chunk = chunkIndex.chunks[index]
            let sanitized = stripInlineReasoningBlocks(from: chunk.summary)
            if sanitized != chunk.summary {
                chunkIndex.chunks[index] = ConversationChunk(
                    id: chunk.id,
                    type: chunk.type,
                    startDate: chunk.startDate,
        ...
        for index in chunkIndex.historicalMetaSummaries.indices {
            let meta = chunkIndex.historicalMetaSummaries[index]
            let sanitized = stripInlineReasoningBlocks(from: meta.summary)
            if sanitized != meta.summary {
                chunkIndex.historicalMetaSummaries[index] = HistoricalMetaSummary(
                    id: meta.id,
                    kind: meta.kind,
                    startDate: meta.startDate,
                    endDate: meta.endDate,
        ...
        if changed > 0 {
            print("[ArchiveService] Sanitized \(changed) stored archive summar\(changed == 1 ? "y" : "ies")")
        }
