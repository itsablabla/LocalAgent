            ...
            Section {
                VStack(alignment: .leading, spacing: 8) {
                    Text("Gmail, Calendar, Contacts, and Drive are reached through the `gws` CLI. Install and authenticate it in a terminal — no credentials live in the app.")
                        .font(.callout)
                    Text("brew install gws && brew install --cask google-cloud-sdk")
                        .font(.system(.callout, design: .monospaced))
                        .padding(6)
                        .background(Color.secondary.opacity(0.1))
                        .cornerRadius(4)
                    Text("gcloud auth login && gws auth setup --login")
                        .font(.system(.callout, design: .monospaced))
                        .padding(6)
                        .background(Color.secondary.opacity(0.1))
                        .cornerRadius(4)
                    Text("The setup command creates the Google Cloud project, enables the Workspace APIs, and runs the OAuth sign-in — the browser only opens to sign in. If the CLI is missing or unauthenticated, the ambient inbox/calendar blocks are silently skipped — the app stays usable.")
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
            } header: {
